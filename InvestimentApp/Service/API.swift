...
protocol APIService {
    func getInvestiment(params:[String:Any], onComplete:@escaping(Investiment) -> Void, onError:@escaping(Error) -> Void)
}
        ...
        let dataTask = URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            if error == nil {
                ...
                if let dictJson = self.parseDataToDictionary(data) {
                    if let model = try? JSONDecoder().decode(Investiment.self, from: JSONSerialization.data(withJSONObject: dictJson, options: .prettyPrinted)) {
                        onComplete(model)
                    }
                }
